...
public final class StaticTextureProvider implements TextureProvider {
    ...
    public StaticTextureProvider(@Nonnull byte[] data, boolean disableLod) {
        ...
        try (NativeImage image = NativeImage.read(buffer)) {
            ...
            if(!disableLod) glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
